...
export async function fetchDistricts(divisionName?: string): Promise<District[]> {
  ...
  const { data: div } = await supabase.from("divisions").select("id").eq("name", divisionName).maybeSingle();
  if (!div) return [];
  const { data } = await supabase.from("districts").select("*").eq("division_id", div.id).order("name");
...
export async function fetchThanas(divisionName?: string, districtName?: string): Promise<Thana[]> {
  ...
  const { data: div } = await supabase.from("divisions").select("id").eq("name", divisionName).maybeSingle();
  if (!div) return [];
  const { data: dist } = await supabase.from("districts")
    .select("id").eq("division_id", div.id).eq("name", districtName).maybeSingle();
  if (!dist) return [];
  const { data } = await supabase.from("thanas").select("*").eq("district_id", dist.id).order("name");
